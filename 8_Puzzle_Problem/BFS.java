...
public class BFS {
    ...
    static int[][] moves = {{-1, 0}, {0, 1}, {0, -1}, {0, 1}}; // up right left down
    ...
    public static List<String> way(Map<int[][], int[][]> parentMap, int[][] curr){
        ...
        while (parentMap.get(curr) != null) {
            ...
            if(parentpos[0] - currpos[0] == 1) path.add("Up");
            if(parentpos[0] - currpos[0] == -1) path.add("Down");
            if(parentpos[1] - currpos[1] == 1) path.add("Left");
            if(parentpos[1] - currpos[1] == -1) path.add("Right");
